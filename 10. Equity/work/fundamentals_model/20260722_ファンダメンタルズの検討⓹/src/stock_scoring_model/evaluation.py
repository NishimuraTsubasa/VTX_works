...
def evaluate_scenarios(
    ...
    if not summary.empty:
        # DataFrame.get() returns the scalar default when the column is absent.
        # In that case, calling .fillna() raises:
        # AttributeError: 'int' object has no attribute 'fillna'.
        # Keep the comparison vectorized by always constructing a Series that is
        # aligned to summary.index, including when no common OOS metrics exist.
        if "CommonEvaluationPeriods" in summary.columns:
            common_periods = pd.to_numeric(
                summary["CommonEvaluationPeriods"],
                errors="coerce",
            ).fillna(0)
        else:
            common_periods = pd.Series(0, index=summary.index, dtype=float)

        summary["CommonOOSWarning"] = np.where(
            common_periods.lt(warning_periods),
