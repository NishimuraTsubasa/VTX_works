...
def read_inputs(config: dict[str, Any], root: Path) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], pd.DataFrame]:
    ...
    return data, sheets, pd.DataFrame()
...
def ensure_output_dirs(config: dict[str, Any], root: Path) -> dict[str, Path]:
    ...
    paths = {"root": out, "diagnostics": out / "diagnostics", "patterns": out / "stock_score_patterns", "history": out / "history"}
