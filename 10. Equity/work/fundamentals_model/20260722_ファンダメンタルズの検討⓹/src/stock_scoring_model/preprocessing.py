    ...
    for code, meta in metas.items():
        ...
        if rank_transform == "gaussian":
            ...
            def zscore(s: pd.Series) -> pd.Series:
                ...
                std = x_num.std(ddof=0)
                return (x_num - x_num.mean()) / std if np.isfinite(std) and std > 0 else x_num * np.nan
            score = temp.groupby(c["date"], group_keys=False)["__x"].apply(zscore)
            score = score * meta.direction
        else:
            score = temp.groupby(c["date"], group_keys=False)["__x"].apply(percentile_rank)
            if meta.direction == -1:
                score = 1.0 - score
            if rank_transform == "centered_percentile":
                score = 2.0 * score - 1.0
