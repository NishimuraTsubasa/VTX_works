...
def fit_regional_models(
    ...
    if config["layer3"].get("training_mode", "rolling_pooled") == "cross_sectional_coefficient_average":
        return rolling_cross_sectional_coefficient_average(
            data, X, penalties, standardize_mask, config, region,
            "regional_pooling", target_col, eligible_rows
        )
    return rolling_pooled_prediction(
        data, X, penalties, standardize_mask, config, region,
        "regional_pooling", target_col, eligible_rows
    )
