...
def build_layer3_design(
    ...
    type_map = feature_types.set_index("Feature")["FeatureType"].to_dict() if not feature_types.empty else {}
    standardize_types = set(
        cfg.get(
            "standardize_feature_types",
            ["factor_basis", "sector_factor_interaction", "country_deviation"],
        )
    )
    standardize_mask = np.asarray(
        [type_map.get(col, "") in standardize_types for col in base.columns],
        dtype=bool,
    )
    if not bool(cfg.get("standardize_continuous_features", True)):
        standardize_mask[:] = False
